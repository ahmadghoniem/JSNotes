...
typeof  {name:"ahmad",age:17,country:"eg"} //  object 
...
mainLoop: for (i = 0; i < myProducts.length; i++) {
  ...
  nestedLoop: for (j = 0; j < productsColor.length; j++) {
    if (myProducts[i] == "keyboard"){ continue mainLoop;} //skips working on the main loop within the nested loop
...
/*
var
redeclare(yes) // you can type var a = 3; var a=4; a will equal now 4 without throwing any errors 
accesss before redeclare(undefined)
...
["1","2"].map( (element, index, array)=> {return element+this }, 10) // this won't work as it will return (2) ['1[object Window]', '2[object Window]']
["1","2"].map(function (element, index, array) {return element+this }, 10) // this will work fine as it going to return (2) ['110', '210'] remember["1","2"] strings
["1","2"].map(function (element, index, array) {return +element+this }, 10) // this will work fine as it going to return (2) ['11', '12'] remember["1","2"] strings
// + unary operator 
...
//current val: the current value of the array being proccessed
//current index: the index of the current value of the element being processed 
//array: the current array
// initial value: if left empty the inital value the acc will have is going to the element with index 0 and (the element being proccessed is element w/ index 1)
// if was provided with a value it will assigned to the acc and the (element being proccessed will be of index 0)
let numbers=[10,20,30,40,50,60];

let reducedNums = numbers.reduce(function(acc,ele,eleInd,arr) {
  console.log(`accumulator value: ${acc} 
  element being proccessed ${ele}
  index of current element being proccessed ${eleInd}`)
  return acc+ele;
},0);
...
let newObjUsingNewKeyword = new Object({}); // using the Object constructor to construct new object
  let neoObject = {
    ...
    nestedObj: {
      ...
      secondNestedObj: {name:"2nd nested obj inside nested obj inside neo obj"}
    },
...
let finalObj2 = { ...obj1, ...obj2, ...obj3 }; // you can do the same functionality by spreading the objects props into a new object
...
element.onclick = func1(); element.onclick= func2(); // func 2 is the one that will be attached only
element.addEventListener("click",func1()); element.addEventListener("click",func2()); // both functions will be executed once the even is triggered
...
let response = confirm("are you sure you wanna do this?");
...
// 
function greet(name, age=0) {console.log(`hello your name is ${name} and age is ${age}`)};
...
let names,ages;
let myoBJ = {names:"ahmad",ages:23,skills:{"1st":"html","2nd":"css"}, skillsArr:["html", "css"]};
({names, ages} = myoBJ) // wrapping this with () to solve the issue
({names:devName, ages:devAge, hobbies="code writing", hobbies:devHob="code writing" } = myoBJ)
...
( { skills: {"1st":first,"2nd":second}, skillsArr:[one,two] } = myoBJ )
first=== one === "html"
second === two === "css"
 ...
 */ 
let myObject = {"name":"sheko","method":function(){}}; // [[Prototype]] : Object already got methods that you can override if you weren't careful such as 
...
email.match(validEmail); ['O@g.com', 'O@g.net', 'A@Y.com', '1@1.com'] 
...
animalMethods = { eat: function () {}, sleep: function () {} };
  ...
  // the problem with the approach above is that you create those 2 same methods for each object which is a waste of memory 
...
class Pen {
  ...
  */
  ...
  // a method in a class is defined using a shorthand method definition syntax, which looks like this:
  MsgForEmployee1() {
...
o = Object.create(null, {  p: 42  }); // ​ Uncaught TypeError: Property description must be an object
...
export const SmallButton = ()=> {
  return <button style={{padding: '10px 20px'}}>small</button>;
}
...
// for named exports 3rd approach
 ...
 function ExtraBigButtons() {
  return <button style={{padding: '40px 60px'}}>Extra Big</button>;
}
// 👇️ named exports 3rd approach
export {SmallButton, ExtraBigButtons};
...
const postToWebPage = (jsonUserData) => {
  jsonUserData.forEach(({ name: n, username: un, email: em }) => {
...
// concurrently (preventing waterfall) but maybe not serialized 
...
/**
 * after myFunc() is called, the program immediately moves on to the next line of code, which is the console.log() statement outside of myFunc().
...
// In this case we’ve dispatched both async actions (setTimeOut is running concurrently like Promise.all() ) before we await anything, so they’re both run concurrently.
...
let callback = ([{intersectionRatio,isIntersecting,target}], observer) => {
